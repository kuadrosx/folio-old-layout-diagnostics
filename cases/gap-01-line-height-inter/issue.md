...
# folio: `line-height: normal` for Inter
...
`line-height: normal`), rendered with Inter instead of Poppins, to guard
against a fix that happens to work for one font's metrics but regresses
another's.

## Minimal repro

`sample.html` — two lines of Inter text at `font-size:11px;
