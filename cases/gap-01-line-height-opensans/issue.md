...
# folio: `line-height: normal` for Open Sans
...
`line-height: normal`), rendered with Open Sans instead of Poppins, to guard
against a fix that happens to work for one font's metrics but regresses
another's.

## Minimal repro

`sample.html` — two lines of Open Sans text at `font-size:11px;
