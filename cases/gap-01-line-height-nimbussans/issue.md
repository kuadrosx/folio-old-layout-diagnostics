...
# folio: `line-height: normal` for Nimbus Sans
...
`line-height: normal`), rendered with Nimbus Sans instead of Poppins, to guard
against a fix that happens to work for one font's metrics but regresses
another's.

## Minimal repro

`sample.html` — two lines of Nimbus Sans text at `font-size:11px;
