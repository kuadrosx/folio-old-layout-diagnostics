...
# folio: nowrap/glyph-advance for Nimbus Sans Bold
...
box containing a long unbreakable token), rendered with Nimbus Sans Bold instead
of Poppins Bold, to guard against a fix that happens to work for one
font's advances but regresses another's.

## Minimal repro

`sample.html` — a 320px box containing a long unbreakable Nimbus Sans Bold token
