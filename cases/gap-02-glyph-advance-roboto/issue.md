...
# folio: nowrap/glyph-advance for Roboto Bold
...
box containing a long unbreakable token), rendered with Roboto Bold instead
of Poppins Bold, to guard against a fix that happens to work for one
font's advances but regresses another's.

## Minimal repro

`sample.html` — a 320px box containing a long unbreakable Roboto Bold token
